...
final class ImageUrlBuilder
{
    ...
    public function waterImg(
        ...
        $qf = new Qiniu\Qfunctions();
        // url合法效验
        if (! $this->isUrl($url)) {
            return $url;
        }

        $waterStr = 'watermark/1/image/' . $qf->base64_urlSafeEncode($image) . '/';
    ...
    public function waterText(
        ...
        $waterStr = 'watermark/2/text/'
            . $qf->base64_urlSafeEncode($text) . '/font/'
            . $qf->base64_urlSafeEncode($font) . '/';
        ...
        if (! is_null($fontColor)
            ...
            $waterStr .= 'fill/' . $qf->base64_urlSafeEncode($fontColor) . '/';
        }
